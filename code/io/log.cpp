...
void Log::add_forward_entry(uint16_t destination_id, uint16_t next_hop_id, const char *message)
{
    char log_line[5000];
    size_t line_length;
...
void Log::write_to_file(const char *log_line, size_t line_length)
{
    fwrite(log_line, sizeof(char), line_length, file_handler);
    fflush(file_handler);
}

void Log::add_send_entry(uint16_t destination_id, uint16_t next_hop_id, const char *message)
{
    char log_line[5000];
    size_t line_length;
...
void Log::add_unreachable_entry(uint16_t destination_id)
{
    char log_line[5000];
    size_t line_length;
